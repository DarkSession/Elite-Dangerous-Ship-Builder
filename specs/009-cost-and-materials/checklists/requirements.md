...
- **The material list is composed, not summed.** The package's per-blueprint cost is already
  ...
  code; the arithmetic is not.
- **Unpriced modules make the totals lower bounds, not unavailable figures** (FR-003). The package's
  modules sum is documented as a floor whenever its unpriced list is non-empty and its rebuy is
  taken from that same floor, so both inherit the bound. A floor is a true and useful statement
  where "unavailable" would discard the prices the catalogue does carry.
...
- **A grand total of material units is deliberately excluded.** A single number adding units of
  selenium to units of iron describes nothing a Commander can act on, so FR-007 requires the count
  of distinct materials instead. No count of rolls, grades or engineering jobs is presented either:
