...
- [x] T061 [P] Satisfy feature 011 FR-015 by stating the qualified conformance wording naming excluded criteria 2.1.1, 2.1.2, 2.1.4, 2.2.1, 2.4.1, 2.4.3, 2.4.7 and 2.4.11 wherever this feature's accessibility conformance is reported in `specs/012-help-and-licences/design/help-and-licences.md`
...
- [x] T074 [P] Implement the shared `InlineLink` presentation component in `src/app/ui/components/inline-link/`: a link inside a sentence rather than a control beside one — no target box, no padding, underlined, wrapping with its line — carrying `target="_blank"` and `rel="noopener noreferrer external"`, with the visible words as the whole accessible name. Register it in feature 011's preview catalogue. Its template is held on one line by `prettier-ignore`, because the whitespace around an inline element is content: a formatter's indentation becomes a real space and draws `GitHub , and`
