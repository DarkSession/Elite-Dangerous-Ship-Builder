...
Feature 001 supplies the application frame, canonical build fragment, browser persistence and the
production app shell. Feature 011 supplies the shared dialog layer, tokens, localisation with bundled
English fallback, the preview catalogue and the ten-project Chromium/Firefox axe harness. Tasks below
extend those owning paths rather than duplicating them; a task that names an 011-owned file assumes
that file exists.
...
- [ ] T001 Add `help:manifest`, `help:manifest:check` and `legal:sync` script entries to `package.json`, and chain `help:manifest` ahead of `start`, `build`, `watch`, `typecheck` and `test` and `help:manifest:check` into `check` so no Angular command runs against a stale generated module
- [ ] T002 [P] Ignore the generated browser module `src/app/platform/build/help-manifest.generated.ts` in `.gitignore` and `.prettierignore` while keeping `LICENSE` and `legal/` tracked
...
- [ ] T006 Define `HelpManifestV1`, `BuildIdentity`, `AlmanacIdentity`, `FrontierDisclaimer`, `ExternalDestination` and `SourceDistributionArtifact` as readonly types plus an `assertHelpManifest` invariant validator in `src/app/domain/distribution/help-manifest.ts`
- [ ] T007 Unit test the invariants — exactly one disclaimer, one destination per ID, `repositoryLicense` as the sole `completeLegalTerms` destination, 64-lowercase-hex `sha256`, positive `byteLength`, non-empty versions — in `src/app/domain/distribution/help-manifest.spec.ts` (depends on T006)
...
- [ ] T010 Implement build-identity classification in `scripts/generate-help-manifest.mjs`: copy `applicationVersion` from root `package.json#version`, emit `kind: "release"` only for explicit version-matched release-workflow evidence over a non-`0.0.0` version, and otherwise emit `kind: "nonRelease"` with a bounded CI identifier or abbreviated commit plus optional `dirty` marker validated against the accepted alphabet (depends on T008)
...
- [ ] T016 [P] Implement the ephemeral `HelpDialogStore` signal store with `HelpInvocationContext`, `HelpDialogState` and the `closed → open → open(replace invocation) → closed` transitions in `src/app/application/help/help-dialog.store.ts`, with a spec proving it touches no Router, History, URL, storage or build state
- [ ] T017 Implement the `HelpPresenter` shell composing the eagerly imported generated manifest with feature 011 localisation into `HelpDialogViewModel.title` and `purpose` in `src/app/application/help/help.presenter.ts`, with `src/app/application/help/help.presenter.spec.ts` asserting there is no loading, empty or legal-error view-model state (depends on T006, T016)
- [ ] T018 Implement the shared modal shell in `src/app/features/help/help-dialog.component.ts`, `help-dialog.component.html` and `help-dialog.component.scss`: feature 011 dialog layer with `role="dialog"`, `aria-modal="true"`, a visible labelled `Help · About` title, purpose, always-visible close action, header pinned over a vertically scrolling body, centered bounded wide treatment and full-width narrow bottom sheet (depends on T017)
- [ ] T019 Mount the single modal instance in feature 011's application frame and add the visible localised `Help · About` action to both the wide frame and the narrow action menu in `src/app/ui/components/app-frame/` so it is reachable from every capability and no-build state (depends on T018)
- [ ] T020 [P] Implement the `ContextHelpLink` presentation component with a visible localised label, shared 44 CSS-pixel target and open-intent output in `src/app/ui/components/context-help-link/`
- [ ] T021 Wire `ContextHelpLink` into package-backed artwork and value regions and into full-screen layers that obscure the frame, so every surface required by FR-002 and FR-011 dispatches the shared open intent instead of embedding help copy (depends on T020)
- [ ] T022 [P] Seed the shell and section-heading message keys — dialog title, purpose, close, `Help`, `Versions and data`, `Licence` — in `src/app/i18n/locales/en.json` and `src/app/i18n/locales/de.json`
- [ ] T023 [P] Register the closed-frame-entry, closed-contextual-entry and open-shell states in feature 011's preview catalogue `src/app/ui/previews/preview-manifest.ts` at desktop, tablet and mobile widths
- [ ] T024 Create the journey harness in `e2e/help-and-licences.spec.ts` covering all four inventory entries — wide frame action from a no-build capability, narrow menu action from an active workspace, package artwork entry and package value entry — asserting exactly one dialog instance, unchanged pathname/query/fragment/history length/build revision/selected slot/stored records, no route chunk or cross-origin request on open, and an unchanged underlying capability after close (depends on T019, T021)
...
**Independent Test**: Open the modal from any capability, including on a first offline visit, and
confirm the disclaimer text is byte-identical to a fresh generator extraction, that it is the only
embedded legal body, and that exactly one action — the repository `LICENSE` on GitHub — is described
as the destination for remaining licence and third-party terms.

- [ ] T025 [US1] Add disclaimer failure fixtures to `scripts/generate-help-manifest.test.mjs` — absent, duplicated, malformed, empty, nested, section-crossing and boundary-crossing blocks, a missing `Under those rules:` marker, invalid UTF-8 and an emitted payload whose bytes, count or hash differ from fresh extraction — each asserting a named non-zero failure and no emitted output (depends on T015)
- [ ] T026 [US1] Add legal-boundary failure fixtures to `scripts/generate-help-manifest.test.mjs` — one-byte mirror drift, an absent or unreadable mirror, a root `LICENSE` that no longer distinguishes the MIT grant from package artwork and game data, a non-HTTPS, credentialed, ported, queried or fragmented licence URL, an unexpected host/repository/ref/path, and an emission carrying more or fewer than one `completeLegalTerms` destination (depends on T025)
- [ ] T027 [P] [US1] Implement the `LegalExcerpt` presentation component in `src/app/ui/components/legal-excerpt/` rendering the source and language framing plus a wrapping text-only `lang="en"` region bound with interpolation, never `innerHTML`, Markdown, an iframe or automatic linking
- [ ] T028 [P] [US1] Implement the `WarnedExternalLink` presentation component in `src/app/ui/components/warned-external-link/` as a native anchor that is inert until activation, carries `rel="noreferrer noopener"`, states its destination purpose plus the leaving-application and possible-network warnings in visible and accessible text, and meets the shared 44 CSS-pixel target
- [ ] T029 [US1] Extend `src/app/application/help/help.presenter.ts` with the `disclaimer`, `disclaimerLanguageNotice` and `repositoryLicense` projections, passing `exactText` through unchanged and taking the URL only from the generated manifest (depends on T017)
- [ ] T030 [US1] Render the Licence section — heading, attribution, original-English notice, `LegalExcerpt`, then the warned repository-`LICENSE` action — as the final block of `src/app/features/help/help-dialog.component.html`, preserving the invariant reading order (depends on T027, T028, T029)
...
- [ ] T032 [P] [US1] Register the exact-disclaimer, warned-external-action, RTL-framing-with-English-excerpt and expanded-text preview states in feature 011's preview catalogue `src/app/ui/previews/preview-manifest.ts`
...
- [ ] T036 [US1] Extend `e2e/help-and-licences.spec.ts` with the offline journey: install the production app shell, disable the network, reload a no-build capability, open the modal before any hull artwork, and assert the complete disclaimer and framing are present with no request and no loading, missing or stale state while the network warning remains visible (depends on T030)
...
- [ ] T037 [US2] Add identity failure fixtures to `scripts/generate-help-manifest.test.mjs` — missing, mismatched and placeholder `0.0.0` release evidence, production optimisation offered as release evidence, a missing or unsafe non-release identifier containing whitespace, a URL, a slash, a branch, a person, a machine, a timestamp or a random value, an installed package name other than `@elite-dangerous-almanac/core`, an empty application or Almanac version, and an absent, changed or unsafe `bugs.url` (depends on T026)
- [ ] T038 [P] [US2] Implement the `VersionFacts` presentation component in `src/app/ui/components/version-facts/` rendering semantic term/definition pairs with separate application and bundled-Almanac labels and a textual release or non-release state that never relies on colour, position or styling alone
- [ ] T039 [US2] Extend `src/app/application/help/help.presenter.ts` with the `identityFacts`, `provenance` and `almanacIssues` projections, always exposing `buildId` for a non-release identity and taking every value from the generated manifest (depends on T017)
- [ ] T040 [US2] Render the Versions and data provenance section between the Help topics and the Licence section in `src/app/features/help/help-dialog.component.html`, composing `VersionFacts`, the provenance notice and the warned Almanac package-defect action (depends on T028, T038, T039)
...
- [ ] T042 [P] [US2] Register the open-release, open-non-release-with-build-ID and long-application/build/package-identifier preview states in feature 011's preview catalogue `src/app/ui/previews/preview-manifest.ts`
...
- [ ] T046 [P] [US3] Define the seven ordered `HelpTopicDefinition` records — `buildLinkPrivacy`, `accountsUploadsTelemetry`, `browserPersistence`, `offlineAssets`, `completedEngineeringGrades`, `hullFactsAndBuildResults`, `almanacOwnership` — with their stable IDs, question and answer message keys and non-bundled documentation evidence links in `src/app/application/help/help-topics.ts`
...
- [ ] T048 [US3] Extend `src/app/application/help/help.presenter.ts` with the ordered `topics` projection and an invariant that rejects a missing, duplicated, reordered or blank topic (depends on T046, T047)
- [ ] T049 [US3] Render the Help topics section directly after the header in `src/app/features/help/help-dialog.component.html` as one reading sequence with a coherent heading hierarchy, and apply the optional invocation topic hint to the initial in-modal position only (depends on T048)
- [ ] T050 [US3] Unit test in `src/app/application/help/help.presenter.spec.ts` that all seven IDs resolve in the required order, that no answer contains a raw key, blank value, interpolation placeholder or HTML, and that the prohibited import and partial-roll wording is absent (depends on T049)
- [ ] T051 [P] [US3] Register the all-seven-topics-populated, doubled/expanded-text and RTL-framing preview states in feature 011's preview catalogue `src/app/ui/previews/preview-manifest.ts`
- [ ] T052 [US3] Extend `e2e/help-and-licences.spec.ts` asserting all seven topics are visible from both a global and a contextual entry, that a contextual topic hint changes only the initial position, and that the complete modal remains reachable in every invocation (depends on T049)
- [ ] T053 [P] [US3] Extend feature 011's locale catalogue gate in `scripts/check-interface-foundations.mjs`, with fixtures in `scripts/check-interface-foundations.test.mjs`, so a build fails when any shipped locale lacks a non-empty question or answer for any of the seven topic IDs, keeping bundled English as the final fallback
...
- [ ] T054 [P] Extend `e2e/help-and-licences.spec.ts` with the axe, semantic and no-overflow sweep over the closed background and every open state — release, non-release, global, contextual, alternate locale and long text — across feature 011's ten Chromium and Firefox viewport and orientation projects
- [ ] T055 [P] Assert the 200%-text and actual-400%-zoom reflow states in `e2e/help-and-licences.spec.ts`: every section and action stays reachable, the title and close stay available, the disclaimer is not clipped and the document has no horizontal overflow
- [ ] T056 [P] Assert in `e2e/help-and-licences.spec.ts` that open and closed state remains immediate and textual under `prefers-reduced-motion` and that no meaning depends on motion, colour, icon, shape, dimming or placement
- [ ] T057 Assert in `e2e/help-and-licences.spec.ts` that opening the already-loaded modal presents its first complete frame within 100 ms under the shared mobile 4x-CPU profile with no route load, same-origin asset request or cross-origin request (depends on T024)
- [ ] T058 [P] Confirm the eagerly imported manifest and bundled English help catalogue stay within the production initial-bundle budget in `angular.json` and adjust the budget entry only with recorded justification
...
- [ ] T060 [P] Record the completed manual screen-reader protocol — entry discovery from a no-build and an active capability, single labelled dialog, background isolation, heading and topic order, version distinctions, disclaimer source and language, external warnings and the unchanged capability after close — in `specs/012-help-and-licences/design/screen-reader-record.md`
...
- [ ] T063 Add this feature's frame-entry, contextual-entry, modal, legal, identity and topic coverage entries with their FR-001–FR-011 requirement ids to `e2e/coverage-ledger.ts`
- [ ] T064 Walk [quickstart.md](./quickstart.md) sections 1 through 8 against the built application and record any divergence as a defect rather than a documentation edit (depends on T054)
- [ ] T065 Run `pnpm run check` to green — format, typecheck, production build, generator tests, unit coverage at or above the 80% thresholds and the complete Playwright and axe matrix — with no skipped browser, viewport, accessibility rule or test (depends on T064)
...
- **User Story 2 (Phase 4)**: Depends on Foundational only; shares `WarnedExternalLink` from T028, so either take US1 first or land T028 before T040
...
- **US2 (P1)**: Independent. Adds the Versions and data provenance section, its generator fixtures and its journeys. Only its shared warned-link primitive overlaps US1
- **US3 (P2)**: Independent. Adds the Help topics section and its journeys. It needs no manifest field
...
- `scripts/generate-help-manifest.mjs`: T008 → T009–T012 → T013 → T014
- `scripts/generate-help-manifest.test.mjs`: T015 → T025 → T026 → T037 → T059
- `src/app/application/help/help.presenter.ts`: T017 → T029, T039, T048
- `src/app/features/help/help-dialog.component.html`: T018 → T030, T040, T049
- `e2e/help-and-licences.spec.ts`: T024 → T034–T036, T044, T045, T052, T054–T057

## Parallel Opportunities

- Phase 1: T002, T003, T004 and T005 run together after T001
- Phase 2: T009, T010, T011 and T012 run together after T008; T016, T020, T022 and T023 run together
- Phase 3: T027, T028, T031 and T032 run together
- Phase 4: T038, T041 and T042 run together
- Phase 5: T046, T047, T051 and T053 run together
- Phase 6: T054, T055, T056, T058, T060, T061, T062 and T063 run together
- Across teams: once Phase 2 completes, one developer takes US1, another takes US2 and a third takes US3; only the shared warned-link primitive needs coordination
...
Task: "Warned external link component in src/app/ui/components/warned-external-link/"
Task: "Licence framing messages in src/app/i18n/locales/en.json and de.json"
Task: "Disclaimer, warned action, RTL and expanded-text preview states in the feature 011 catalogue"
```
...
4. **STOP and VALIDATE**: the modal opens from every entry without navigation, the disclaimer is
   byte-identical to root `LICENSE`, one warned GitHub action carries all remaining terms, and it all
   works on a first offline visit
...
- Every component task includes its unit test, its preview states and its message keys; none of the
