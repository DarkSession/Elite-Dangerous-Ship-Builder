...
- [ ] T001 Add `help:manifest`, `help:manifest:check`, `help:topics:check` and `legal:sync` script entries to `package.json`, chain `help:manifest` ahead of `start`, `build`, `watch`, `typecheck` and `test`, and chain both check commands into `check` so no Angular command runs against a stale generated module or invalid help-topic set
...
- [ ] T006 Define `HelpManifestV1`, `BuildIdentity`, `AlmanacIdentity`, `FrontierDisclaimer`, `ExternalDestination`, `SourceDistributionArtifact`, `HelpTopicId` and the browser-safe topic record containing only ID/question/answer message keys as readonly types plus an `assertHelpManifest` invariant validator in `src/app/domain/distribution/help-manifest.ts`
...
- [ ] T018 Implement the shared modal shell in `src/app/features/help/help-dialog.component.ts`, `help-dialog.component.html` and `help-dialog.component.scss`: feature 011 dialog layer with `role="dialog"`, `aria-modal="true"`, a visible labelled `Help · About` title, purpose, always-visible close action, header pinned over a vertically scrolling body, centered bounded wide treatment and full-width narrow bottom sheet; add focused shell-semantic coverage in `src/app/features/help/help-dialog.component.spec.ts` (depends on T017)
...
- [ ] T020 [P] Implement the `ContextHelpLink` presentation component with a visible localised label, shared token-backed 44 CSS-pixel target and open-intent output, plus its focused co-located unit spec, in `src/app/ui/components/context-help-link/`
- [ ] T021 Wire `ContextHelpLink` into every package-backed artwork/value region and every full-screen layer enumerated by `specs/012-help-and-licences/design/screen-inventory.md`, including `src/app/features/hull-detail/hull-detail.page.html`, `src/app/features/build-workspace/build-workspace.page.html` and `src/app/ui/components/layer/layer.html`, so every surface required by FR-002 and FR-011 dispatches the shared open intent instead of embedding help copy (depends on T020)
...
- [ ] T024 Register the closed-frame-entry, closed-contextual-entry and open-shell states in feature 011's preview catalogue `src/app/ui/previews/preview-manifest.ts` at desktop, tablet and mobile widths (depends on T019, T020, T021, T022)
- [ ] T025 Create the exhaustive FR-011 coverage set in `e2e/coverage-ledger.ts` and the journey harness in `e2e/help-and-licences.spec.ts`: enumerate and open help from every current capability, applicable package-backed artwork/value surface and obscuring layer in `specs/012-help-and-licences/design/screen-inventory.md`, including at least the wide frame action from a no-build capability, narrow menu action from an active workspace, package artwork entry and package value entry; assert exactly one dialog instance, unchanged pathname/query/fragment/history length/build revision/selected slot/stored records, no route chunk or cross-origin request on open, and an unchanged underlying capability after close (depends on T019, T021, T024)
...
- [ ] T048 [US3] Implement `scripts/check-help-topics.mjs` and `scripts/check-help-topics.test.mjs` to reject a missing or duplicate required ID, changed order, empty or unresolved governing-reference set, unknown requirement/principle target, missing/blank shipped-locale question or answer and mismatched interpolation variables; extend `scripts/generate-help-manifest.mjs` to emit only the validated ID/question/answer key subset and no governing references (depends on T046, T047)
- [ ] T049 [US3] Extend `src/app/application/help/help.presenter.ts` with the ordered generated `topics` projection and a defensive invariant that rejects a missing, duplicated, reordered or blank runtime topic without publishing a partial modal (depends on T048)
...
- [ ] T051 [US3] Unit test in `src/app/application/help/help.presenter.spec.ts`, `src/app/domain/distribution/help-manifest.spec.ts` and `scripts/generate-help-manifest.test.mjs` that all seven IDs resolve exactly once in order, no answer contains a raw key, blank value, unresolved interpolation or HTML, prohibited import/partial-roll wording is absent, and neither a governing reference nor a partial topic set enters generated browser output (depends on T050)
...
- **US3 (P2)**: Independent. Adds the governed Help topics to the generated manifest, modal section
  and journeys without bundling the governing references
...
- `scripts/generate-help-manifest.mjs`: T008 → T009 → T010 → T011 → T012 → T013 → T014 → T048
- `scripts/help-topic-definitions.mjs` and `scripts/check-help-topics.mjs`: T046 → T048
- `scripts/generate-help-manifest.test.mjs`: US1's T026 → T027, US2's T037 and US3's T051 each branch from T015 and merge one at a time; T059 follows T037
- `src/app/application/help/help.presenter.ts`: T017 → T029, T039, T049
- `src/app/features/help/help-dialog.component.html`: T018 → T030, T040, T050
...
- Phase 2: T016, T020, T022 and T023 run together; generator edits T009–T014 remain sequential
