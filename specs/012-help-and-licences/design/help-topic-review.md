...
**Provisional, 2026-08-25.** The judgment below was made against the settled wording for T047 —
the seven question and answer pairs that task is to enter into the catalogues. Neither catalogue
carries a `help.topic.*` key yet, so this record judges text that has been decided and not yet
shipped, and that is the one thing it cannot yet prove.

**Confirm it against `src/app/i18n/locales/en.json` and `de.json` the moment T047 lands**, and
strike this paragraph then. Afterwards, re-run the review whenever an English question, an English
answer or a governing reference changes; a topic left unchecked is a release failure, not a note to
come back to.
...
## 1. `buildLinkPrivacy` — "What does a shared build link contain?"

**Governed by** feature 001 FR-015.

| Check | Finding                                                                                                                                                                                                                                                                                                        |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | FR-015 requires the payload to stay entirely in the URL fragment and to cause no transmission. The answer states both halves — where the payload is, and that a browser does not send that part — and adds only the ordinary consequence that handing somebody the whole address hands them the encoded build. |
...
| 4     | German says the same three things in the same order and uses the shipped `Aufbau-Link` vocabulary.                                                                                                                                                                                                             |
...
## 3. `browserPersistence` — "Where are builds and preferences stored?"
...
| 4     | German uses the shipped `Aufbau` / `Speicher des Browsers` vocabulary and keeps the same remedy.                                                                                                                                                                                      |
...
## 5. `completedEngineeringGrades` — "How are engineering grades represented?"
...
| 3     | "Refused whole, before anything is opened" is the behaviour feature 004 ships today, asserted by the refusal journey in `e2e/help-and-licences.spec.ts`.                                                                                                                   |
| 4     | German states the same invariant and the same atomic refusal.                                                                                                                                                                                                              |

## 6. `hullFactsAndBuildResults` — "What is a hull fact versus a build result?"
...
## 7. `almanacOwnership` — "Where do game values and calculations come from?"

**Governed by** constitution principle II and feature 003 FR-002.

| Check | Finding                                                                                                                                                                                                                                                        |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | Principle II makes the Almanac the source of truth for catalogue data, validation and calculation; FR-002 is where feature 003 reads its results from it. The answer says exactly that, and that this application neither maintains nor corrects those values. |
...
- **"What can I import?"** promises an import behaviour this application does not own; feature 004
  states what it accepts, and a help answer that generalised it would be a promise nothing keeps.
