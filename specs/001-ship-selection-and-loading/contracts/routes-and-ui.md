...
| `/ships/:hull`                | Package hull detail, addressed by the hull's name with an underscore for each space; a hull symbol is accepted and replaced by that address; wide inspector or narrow full-screen layer; an address no hull answers to is a safe error                                                                           |
