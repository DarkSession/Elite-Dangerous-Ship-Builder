...
**Primary Dependencies**: Angular 22.1 standalone and zoneless APIs, Angular Router, Angular service worker, RxJS 7.8, `@elite-dangerous-almanac/core` 0.1.0-beta.12 leaf exports, Web Storage, Web Locks, BroadcastChannel, History and URL APIs
...
| II. Almanac Source of Truth             | Hulls, facts, slots, defaults, artwork identity, validation and build construction use documented leaf imports. The app derives no game values.                    | PASS   |
...
| IV. Lossless, Honest Builds             | Explicit DTOs preserve absence and unresolved identities; candidate-first loading leaves current work unchanged on failure; unavailable values remain unavailable. | PASS   |
...
- A purpose-built `BuildSnapshotV1` preserves only modelled state, including unresolved entries and package engineering identities; `ShipLoadout.toLoadoutEvent()` is not used wholesale because it adds derived fields and normalizes spelling.
...
The Phase 1 artifacts introduce no server, private game catalogue, derived game calculation, component-owned domain state, hard-coded display text, visual literal or silent fallback value. Local notes and record identities are excluded explicitly from link/SLEF boundaries. Every mutation that could replace work is candidate-first and confirmable; every persistence failure keeps the active build usable. All screen requirements map to the shared design system and dual-engine accessibility matrix. The gate remains **PASS** with no complexity exception.
