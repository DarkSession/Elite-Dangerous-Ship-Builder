...
- The reference PNGs and “27 of 48 covered” copy are stale mock assets/state. 0.1.2 provides an `illustration.svg` for every current hull. Runtime copies those package files to a same-origin asset path and retains defensive loading/missing states for later releases/offline conditions.
