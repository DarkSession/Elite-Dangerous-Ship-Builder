...
| Detail on its way                | The first hull opened from the manifest waits for the screen's chunk. The inspector draws a skeleton where the hull will be. An empty rail with a tab stop in it is not drawn. A hull opened straight at its own address is the frame's case: the catalogue does not exist yet, and the page skeleton stands over the whole screen.                                                                                                                                |
| Detail did not arrive            | The screen's chunk was refused. The inspector states it in place of the hull, as an error the reader is sent to, and the words name a fresh document as the way out: the browser records an import that failed and refuses the same address from that record. The manifest stays drawn, because no screen is coming to take its place: beside the sentence at the rail's width, and under it below that step, where the sentence leads because it is what changed. |
