...
2. Below the manifest rail's own width — every width under 1024, the 768–1023 band included — verify stacked hull records, full-screen detail/library layers, workspace overflow actions and sheet dialogs preserve every action and route. At 900 the hull detail is the full-screen layer with the command bar's return group over it, not a panel under the manifest; a rested pointer opens nothing there, the press does, and the sheet is drawn as a bounded column rather than at the width of the window.
...
4. Confirm all mandatory adaptations: complete FR-004 facts, working/error/storage states, semantic controls, 44 px targets, package SVGs/data, localized formatting, same-origin fonts/assets and canonical `/outfitting#b.…` links.
