...
**Decision**: Import `SHIPS`, `getShipBySymbol`, `getShipSlots` and type `Ship` from `@elite-dangerous-almanac/core/ships/ships`. Present the package fields directly and use the package `symbol` as every hull key. Preserve an explicit unavailable state even though the package currently populates all required facts.
...
**Rationale**: The slot helper preserves irregular game-owned keys; the live factory creates an independent, calculated stock build using the complete catalogue. All catalogued hulls currently have complete valid defaults, while the null/error states remain required defensive behavior.
...
**Decision**: Rasterise each package `assets/ships/*/illustration.svg` to a 900px-wide PNG with `scripts/convert-ship-artwork.mjs` and commit the result under `public/assets/ships/<symbol>/illustration.png`, which the build copies to the same-origin `/assets/ships/<symbol>/illustration.png` path. Cache the app shell and bundled English messages eagerly; cache hull illustrations on first request with the service worker. Reserve artwork aspect ratio, keep creation independent of image state, and have the artwork coordinator retry a failed uncached request when the browser reports that connectivity returned so no page reload is required. Serve the loading mark from the same origin as `public/assets/loader.svg`, a committed copy of EDAssets' `EDLoader1.svg` rather than a link to it.

**Rationale**: The package documents these static files and carries one illustration for every hull, but they are large vector drawings that the browser re-rasterises on every resize of a fixed-ratio plate. A PNG at the plate's own resolution is roughly a tenth of the bytes and costs nothing to draw, and the reference's own artboards use PNGs for the same reason. Committing the output rather than converting during the build keeps `pnpm run build` hermetic and free of an ImageMagick dependency; the script is how the files are reproduced after a package upgrade. Same-origin lazy caching satisfies the constitution without placing every image on the initial critical path.

**Alternatives considered**: Serving the package SVGs directly was the earlier decision and was replaced on 2026-08-21. Converting during the build was rejected because it would make a hermetic build depend on a native tool. Importing SVGs as JavaScript was rejected because the package intentionally exposes them as static assets. Cross-origin/CDN access, including hot-linking the loader, was rejected. Eagerly loading every illustration was rejected because detail artwork is optional presentation weight.
...
**Rationale**: The package supplies locale-result helpers for hull names, manufacturers and
...
- The Almanac supplies hull and structured diagnostic locale-result leaves; an explicit `null`
