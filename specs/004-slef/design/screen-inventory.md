...
| [Import aftermath](./import-outcome.md) | Feature 003's build-status rail and the workspace itself, on `/build` | Disclose the package verdict after the input layer closes — through the surfaces that already draw it; the canvas draws no feature-004 report | no changes, valid, invalid, revision changed                                                                                                                                                                                                         |
...
Successful import from `/ships`, `/ships/:hull` or `/builds` navigates to `/build` only after the
...
Host Import intent
  ...
  -> one commit -> /build; feature 002's completion notice and feature 003's rail
