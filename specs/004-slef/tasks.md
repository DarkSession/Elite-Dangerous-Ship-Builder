...
- **Pinned package**: `@elite-dangerous-almanac/core@0.1.4` leaf subpaths `ships/slef`,
...
- [ ] T001 Pin the Almanac 0.1.4 SLEF behaviour this feature composes — `inspectSlef` accepting one envelope, a one-element array and a bare journal `Loadout` as one valid entry, `[]` as zero, mixed input as one entry plus one indexed diagnostic and malformed JSON as a thrown `SyntaxError`; frozen `SlefDiagnostic` `index`/`path`/`code`/`constraint`/`params`/`message`; `getSlefDiagnosticMessage` returning canonical English and `null` outside English; `ShipLoadout.fromLoadout(entry.data)` unknown-hull refusal and package-populated fixed mounts; `completeEngineeringGrade` returning `normalized`/`unsupported`/`unchanged`; and `toSlefString({ moduleOrder, explicitPower, indent, header })` emitting exactly one entry — using leaf subpath imports in `src/app/domain/slef/almanac-slef-contract.spec.ts`
