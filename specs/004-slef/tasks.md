...
- [ ] T006 [P] Implement the test fixture builder that discovers the installed package's maximum-slot hull at test time and fits every slot with every supported modelled field — enabled state, zero-based priority, ordinary and identified pre-engineered blueprints with effects, ship name and ident — through package APIs only, in `src/app/domain/slef/testing/max-slot-fixture.ts`
...
- [ ] T014 [P] [US1] Add current-retail credit tests proving hull and fitted-module credits, rebuy and totals come from the package's default catalogue-retail calculation after engineering, symbol replacement/removal and package-defaulted fixed construction, that unpriced entries remain absent or lower-bound exactly as reported, that `UnladenMass`, `CargoCapacity`, `FuelCapacity` and `MaxJumpRange` are package values rather than echoed source claims, and that no captured or historical purchase value is requested, read, retained or compared, in `src/app/domain/slef/slef-export-pricing.spec.ts`
...
- [ ] T038 [US1] Add payload exclusion tests proving the artifact carries no local record identity, name or note, no import report, diagnostic, request token or application normalization metadata, and that a package-defaulted fixed mount exports only the resulting module, in `src/app/domain/slef/slef-export-exclusions.spec.ts` (depends on T021)
...
after while preserving the exact draft; and the accepted revision's outcome reports quality
completions and final package validation.
...
- [ ] T048 [P] [US2] Add import outcome component tests for no quality completion, quality completed, retained invalid, combined quality/validation groups, dismissed and revision changed, asserting separately headed groups and dismissal changing presentation only, in `src/app/features/slef/import-outcome/import-outcome.spec.ts`
...
- [ ] T055 [US2] Implement source evidence capture recording resolved `SourcePartialEngineering` with exact slot, module symbol, blueprint/experimental identities, grade and finite quality in `[0, 1)` in `src/app/domain/slef/slef-source-evidence.ts` with unit tests (depends on T004, T052)
...
- [ ] T058 [US2] Assert the fixed-mount invariant after package construction and remove any application repair, default lookup, before/after comparison or provenance projection in `src/app/domain/slef/slef-import.ts` (depends on T057)
- [ ] T059 [US2] Read final package validation and issues only after construction and quality completion, then assemble the detached `SlefImportCandidate` with quality completions, validation issue projections, package-validated source attribution and request token, ignoring incoming `appCustomProperties` and `appURL` entirely, in `src/app/domain/slef/slef-import.ts` (depends on T058)
...
- [ ] T062 [US2] Implement the handoff giving the candidate and token to feature 001's `ReplacementCoordinator`, performing no direct active-state, persistence, URL or history mutation, in `src/app/application/slef/slef-import.coordinator.ts` with unit tests (depends on T059, T061)
...
- [ ] T065 [US2] Publish the revision-bound `SlefImportOutcome` after commit and drop it when the active revision no longer matches, keeping quality completions and the detailed validation issue list out of `BuildSnapshotV1`, the fragment, SLEF and edit history, in `src/app/application/slef/slef.store.ts` with unit tests (depends on T062)
- [ ] T066 [US2] Keep quality completions transient and persist only feature 001's accepted `valid`/`complete` record metadata; fixed defaults carry no provenance in `src/app/application/slef/slef-import.coordinator.ts` (depends on T062)
...
- [ ] T077 [US2] Add the atomic-rejection and replacement journey — seed dirty active state, revision, working and named record bytes, fragment/link and undo/redo, then assert byte equality and a preserved draft after every failure, cancellation and supersession, and that cancelling a ready candidate changes nothing while accepting it performs exactly one replacement, autosave, link synchronization and history reset — in `e2e/slef-import.spec.ts` (depends on T049, T062)
- [ ] T078 [US2] Add the normalization-order and outcome journey — absent and unusable fixed entries plus ordinary, Mercenary and identified partials on resolved modules — asserting package construction first, quality completion next, no repair/provenance branch, whole refusal for unsupported partials, and that dismissal edits nothing while feature 001 persists only the accepted revision's `valid`/`complete` booleans, in `e2e/slef-import.spec.ts` (depends on T049, T070)
