...
A Commander can read what the build costs in credits and what engineering materials it needs,
without leaving the status report.

**Independent Test**: Compare the report's credit and material summary with the Almanac retail and
material results for assembled, imported, unpriced and unengineered reference builds.

**Acceptance Scenarios**:

1. **Given** an active build, **When** status is shown, **Then** the catalogue-retail hull value,
   fitted-module value, rebuy and the consolidated engineering-material requirement follow the
   headline figures in that reading order.
...
- A genuine zero — no cargo capacity, no engineering materials — is shown as zero or as an empty
  requirement, never as unavailable.
...
- **FR-020**: The report MUST present the catalogue-retail hull value, fitted-module value, rebuy
  and the consolidated engineering-material requirement, after the headline figures in that reading
  order, under the rules in [Cost and Materials](../009-cost-and-materials/spec.md).
- **FR-021**: Where the package reports unpriced fitted modules, the affected totals MUST be
  identified as lower bounds and a matching finding MUST name the unpriced slots.
- **FR-022**: A build with no ordinary engineering MUST show an empty material requirement rather
  than an unavailable one.
- **FR-023**: Credit and material entries MUST lead to their detail in one interaction.
...
- **FR-025**: A change in the findings MUST be announced once to assistive technology, without
  re-announcing unchanged figures.
...
- **FR-031**: Unit tests MUST compare the credit and material summary with the Almanac retail,
  blueprint-cost, effect-cost and material-summing results, including unpriced lower bounds and the
  empty requirement.
...
- **Readiness verdict**: The build's flyability as stated by the Almanac's valid and complete flags.
...
- **Requirement summary**: The build's catalogue-retail credit values and consolidated engineering
  materials.
...
headline figures and their availability states; `retailCredits()`, `getBlueprintCost()`,
`getExperimentalEffectCost()` and `sumMaterials()` supply the credit and material summary. The
package documents its English issue text as a log fallback rather than display text, so composing
and translating a finding's wording from the code and parameters is the intended use; game nouns
inside that wording remain package text. No status value in scope requires an application-owned game
calculation.
...
- "Cost" is catalogue retail — hull, modules and rebuy. Captured purchase provenance stays with SLEF
  import and export and is never combined with retail.
...
- **SC-003**: Every displayed figure, credit value and material quantity equals the corresponding
  Almanac result for the same build and viewing conditions.
...
- **SC-006**: A build change updates the verdict, the findings and every affected figure within
  100 ms on the supported mobile performance profile, with no state mixed from a previous build.
