...
- **FR-003**: A `null` shield result MUST remain unavailable, and a Commander MUST be told the
  ...
  generator that is not fitted, one switched off, and one the power plant does not feed MUST each
  be stated in the Commander's own language and MUST remain distinguishable from each other. The
  statement MUST be chosen by the package's own field and reason and by nothing else. Every way the
  plant leaves the generator unfed MAY share one statement, because they are one thing to act on.
  Any other diagnosis MUST keep the package's own words.
