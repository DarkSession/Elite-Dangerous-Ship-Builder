...
The reasons themselves read badly. For most reasons the package builds its sentence from a raw slot key and a
module symbol — `Slot03_Size6: Int_ShieldGenerator_Size6_Class1 is not powered with hardpoints
retracted` — and `getCalculationIssueMessage()` resolves it for English alone whatever the reason,
so every other locale reads the canonical fallback for the most common shield state there is.
...
- [x] T100 Cover the change in `defence-analysis.spec.ts` and `e2e/defence.spec.ts`: one statement
      ...
      refused on its own still stated, and a reason with no entry in the card's table still in the
      package's own words
- [x] T101 Re-run the feature's e2e specs with the axe scan, then `pnpm run check`. Every stage
      ...
      all three
- [x] T102 State the refusals a Commander reaches by outfitting in their own language:
      ...
      given to act on, and principle II sanctions labelling a package result. Both ways the plant
      leaves the generator unfed share one statement, because they are one thing to act on; nothing
      fitted, switched off and unfed stay three. Every pair with no entry keeps the package's
      sentence and its canonical-language disclosure
- [x] T103 Drop the `Shield strength · Unavailable` sentinel where a reason is drawn under it
      (Commander request). The reason states the absence, and the card's heading already says which
      value is missing. `defence.shield.unavailable` and the card's `UnavailableValue` import go
      with it: the package types an incomplete result as carrying one issue at least, so there is
      no refusal without a reason for the sentinel to fall back to
