...
- [x] T006d Add `withEnabledBlockingInitialNavigation()` to `provideRouter` in `src/app/app.config.ts`. **Found during implementation**: every screen is behind a lazily loaded route, so without a blocking initial navigation the router reaches the outlet before that route's chunk has arrived, throws away the document's copy of the screen and draws its own a moment later. Measured on `/ships/Anaconda`: the whole inspector — manufacturer, speed, shield, hull mass, every hardpoint count — left the page for one frame and came back, 647 characters gone and returned. On a phone it was worse: the hull sheet was replaced by the whole catalogue, 1029 pixels tall becoming 4857. T006b makes the takeover adopt the document; this is what gives it a document still there to adopt. Applied outside development only, the same ruling as the service worker: Angular's NG05001 calls hydration and a blocking initial navigation a contradiction, and on a development server — where nothing was rendered — it is one (FR-009, SC-003)
- [x] T006e Set `"prerender": false, "ssr": false` in the `development` configuration in `angular.json`. **Found during implementation**: the configuration inherits the build options, so `ng build --configuration=development` tried to prerender, failed outright on a clean checkout with `ENOENT: dist/prerender-routes.txt`, and — where the file happened to exist — put generated documents under the dev matrix, where 20 journeys then failed against a first frame they were never written for. The dev matrix has no service worker for the same reason and by the same mechanism; this is that precedent applied to documents
- [x] T006f Simplify `betaFollowsTitle` in `src/app/ui/components/app-frame/app-frame.ts` to `identity() === null`. **Found during implementation**: the bar drew a second BETA chip at 155ms and removed it at 1281ms, shifting the product name 36px sideways for a second — a mark appearing and vanishing in the middle of what a Commander is reading (FR-009, SC-003)
- [x] T006g Write the hidden-label class into the markup with `@if`/`@else` in `text-field.html`, `choice-group.html` and `textarea-field.html` instead of binding `[class.field__label--hidden]`. **Found during implementation**: Angular re-initialises a class binding on the first pass after adopting a document, so for one frame every hidden label was visible and all 48 catalogue rows jumped 16px. A class written into markup survives the takeover; a bound one does not (FR-009, FR-010, SC-003)
- [x] T006h Select the returning composition in `app-frame.scss` with `:host(:has(.frame__return))` rather than the `frame--returning` host class. **Found during implementation**: the host class follows `back()`, which is published from a lazily loaded component's effect, so on a mobile hull detail the bar re-composed 46px lower about 1.1 seconds in. The markup already says whether there is a return control; asking it directly needs nothing to have loaded (FR-009, FR-010, SC-003)
...
- [x] T022a [US1] Read documents with a parser rather than with regular expressions in `check-prerendered-documents.mjs`. **Found by CodeQL after the fact**: `js/bad-tag-filter` is right that `<script[\s\S]*?</script>`, `<!--[\s\S]*?-->` and `<[^>]+>` are wrong about the markup they claim to read, and this gate reads a quarter of a megabyte of exactly the cases they get wrong. A gate nobody can believe is worse than no gate The published-address journey carried a second copy of the same readers; both now go through `e2e/served-document.ts`, which parses with `DOMParser` in the browser the suite already has open (research decision 20)
...
- [x] T026 [US2] Create `e2e/prerendered-first-frame.spec.ts`: for each of the three screens, assert the document states its subject to a reader that runs no script, that it states it in a frame the application has not reached yet, that no frame between first paint and interactive is emptier than the one before, and that four boxes — the shell's bar, `main`, the screen, the page — do not move across the takeover (FR-008, FR-009, FR-010, SC-003, SC-004). Boxes rather than `layout-shift` entries, which are Chromium-only while half the matrix is Firefox
- [x] T027 [US2] **Add `prerendered-first-frame.spec.ts` to the `e2e:offline` script's explicit spec list in `package.json:27`.** That script names its files rather than globbing, so without this edit CI runs the new job and never runs the new test
...
- [x] T029 [P] [US2] Add the FR-011 case: the committed locale replaces the document's text without reordering anything on the page or removing anything from it, and reflow is permitted because a translation is not the same length as its source. The English shape is read from the served document rather than from the page before the takeover, because a page is only English for a few hundred milliseconds and a test that races that window reports the language rather than the layout
- [x] T030 [P] [US2] Add the FR-012 case: with the bundle blocked, the Commander is left with the readable document rather than an empty page
- [x] T029a [P] [US2] Amend FR-011 and add FR-011a to [spec.md](./spec.md), and allow the addition in the German case. **Found during implementation**: FR-011's "MUST NOT add, remove or reorder anything" was factually wrong. Every game noun the catalogue names is published in English only, so a German Commander is told so beside each one — 192 visible `game-text__disclosure` spans on `/ships` that the English document does not carry and must not. Suppressing them was the alternative and it is not available: a value shown in a language the Commander did not ask for has to say so, which is `GameText`'s reason for existing. The requirement now names the disclosure as a bounded second exception and the test subtracts exactly it before comparing the two shapes
- [x] T031 [US2] Add the axe scan over the generated first frame, with script execution disabled, across all ten Playwright projects — WCAG 2.2 AA less criteria 2.1.1, 2.1.2, 2.1.4, 2.2.1, 2.4.1, 2.4.3, 2.4.7 and 2.4.11, the same eight the rest of the suite excludes (FR-019, SC-005)
- [x] T032 [P] [US2] Guard `e2e/offline-privacy.spec.ts:61` against the double-count hazard its exact-48 `[data-hull-symbol]` assertion now carries — a prerendered catalogue and a hydrated one across the takeover (design/first-frame.md, "Watch")
...
- [x] T037 [P] [US3] Extend `e2e/offline-privacy.spec.ts` with the repeat-visit case: online, a second visit to `/ships/Anaconda` gets the hull document and never paints the start page's content
- [x] T038 [P] [US3] Extend `e2e/offline.spec.ts` with the unmatched-address case: `/ships/NotAShip` offline gets the shell and the application's own handling, never the start page (FR-015, FR-016)
- [x] T039 [P] [US3] Add the FR-007 assertions to `check-prerendered-documents.mjs`'s prohibition check: no build, no saved record, no Commander data, no runtime environment configuration, and no application version — the version is stamped in CI immediately before `ng build`, so a body carrying it would bake a CI-only value into static HTML (research decision 14, SC-008)
- [x] T040 [P] [US3] Assert FR-017 explicitly in the end-to-end suite: a shared build's payload is in the fragment, and no part of it is in the path or query
...
- [x] T043 [P] Register feature 015's rows in `e2e/coverage-ledger.ts` and add `'015-prerendered-documents'` to `COVERED_FEATURES`, which immediately requires every requirement id the spec declares — FR-001 through FR-021, FR-009a included (constitution VIII)
- [x] T044 [P] Update `README.md`'s deployment section for the new output shape: 50 documents, `index.csr.html` as the shell, and `404.html` copied from it
- [x] T045 [P] Record the measured document sizes and the CI cost of the `e2e-production` job in `research.md`, replacing the spike figures with the shipped ones
- [x] T046 Run `pnpm run check` — format, help artifacts, sitemap check, typecheck, build, preview build, policy, codec capacity, script tests, unit tests at the 80% floor, Playwright, timing and offline — and confirm it is green before proposing the change. Green: everything through `pnpm run test` was run here (484 script tests, 2,897 unit tests, 89.82% statements against the 80% floor), and the three Playwright stages in CI, which is where they can be run at all — half the matrix is Firefox and this container cannot install it (the browser CDN is unreachable), so a local `pnpm run check` would report five projects and claim ten
- [x] T047 Walk [quickstart.md](./quickstart.md) end to end, including the manual service-worker protocol in section 7, and correct anything the implementation moved. Walked; four steps corrected — §1's document count was a placeholder rather than a command, §2 expected the spike's `ships/Adder/index.html` and a heading claim the flat layout makes trivially true, §5 asked a reader to add an unrecorded address by hand when two script tests already assert the gate bites, and §7's manual protocol now records what driving it reported
