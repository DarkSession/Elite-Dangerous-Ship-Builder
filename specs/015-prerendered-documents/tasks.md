...
- [x] T022a [US1] Read documents with a parser rather than with regular expressions in `check-prerendered-documents.mjs`. **Found by CodeQL after the fact**: `js/bad-tag-filter` is right that `<script[\s\S]*?</script>`, `<!--[\s\S]*?-->` and `<[^>]+>` are wrong about the markup they claim to read, and this gate reads a quarter of a megabyte of exactly the cases they get wrong. A gate nobody can believe is worse than no gate (research decision 20)
