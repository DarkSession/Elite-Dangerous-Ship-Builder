...
| `NormalisationProvenanceList` | one/many entries                       | region omitted                      | pending parent state         | malformed metadata rejected by feature 001 | cleared entry absent                                  | separate local-workflow labeling                 |
| `PowerHeadline`               | exact and all owner qualifications     | N/A                                 | pending                      | owner unavailable and app failure parent   | detail action unavailable only while pending          | deployed/retracted, unknown draws, zero/Infinity |
...
| `StatusCapability`            | full ready projection                  | no issues/qualifications/provenance | current revision pending     | application failure                        | no active build uses workspace empty state            | all result combinations and target behavior      |
