...
- **Two range controls, of two different kinds.** Damage at range is charted at five fixed ranges
  identical for every build, so two loadouts read against one scale; convergence takes a continuous
  slider, because the question there is where the spread becomes acceptable. Neither chart nor plot
  may be the only route to its figures.
