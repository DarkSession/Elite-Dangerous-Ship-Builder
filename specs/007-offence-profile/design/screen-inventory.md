...
| `WEAPONS` collection                           | The canvas's five columns where the block is given 31rem, all five sharing its width in the canvas's own `2fr 1fr 1fr 1fr 1fr` proportion, each row inert; labelled cards carrying the same five fields below that, still inert (`design/canvas-contract.md`, "Canvas revision, 2026-08-25") | Labelled cards carrying the same five fields, still inert | FR-001–FR-005                  |
...
| `SHOT CONVERGENCE`                             | The gunsight plate carrying every hardpoint the hull has with the selected one ringed, its shot sentences, the range control, four facts                                                                                                                                                     | The same, the plate narrowing with the block              | FR-001, FR-010–FR-013          |
...
| FR-013      | The mount the workspace has selected is ringed on the plate and named as selected in its own sentence, from the ledger's own slot key.             |
