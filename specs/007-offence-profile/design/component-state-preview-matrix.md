...
| Weapon row             | Damage per second, piercing, maximum range and falloff, all present                                                                   | N/A: a row exists only for a returned weapon                                                   | Genuine zero; disabled; each optional field absent; and inert in every one of them                                           |
...
| Shot convergence       | A placed gunsight: a dot and its hardpoint numeral per mount, armed or empty, with a sentence for each and a ring on the selected one | A placed hull with nothing armed: axes, rings, every mount drawn empty, none of the four facts | Gunsight unpublished for the hull; a mark clamped to the frame's margin; an empty hardpoint selected; the range at both ends |
...
- a hull with some hardpoints armed and the rest empty, so the plate carries both kinds at once;
- a selected hardpoint that is armed, and one that is empty.
