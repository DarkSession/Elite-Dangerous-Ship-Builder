...
| Shot convergence       | A placed gunsight: one dot per mount, armed or empty, in three fills, with a sentence for each | A placed hull with nothing armed: axes, rings, every mount it can hold drawn empty | Gunsight unpublished for the hull; a shot outside the field of view, stated but not drawn; an empty hardpoint selected; the range at both ends |
