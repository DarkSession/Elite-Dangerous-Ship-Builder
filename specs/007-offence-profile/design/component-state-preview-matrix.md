...
| ----------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------- | --------------------------------------- | --------------------------------------- | -------------------------------------------------------------------------------------------- |
| Offence capability composition      | Complete totals, capacitor and multiple weapons        | Workspace no-build is feature 001 owned; confirmed no-weapons ready state | Current revision, no stale figures      | Projection or missing-port alert        | Partial/unresolved coverage; expanded language; RTL; reduced motion                          |
...
| Distributor observation             | Powered exact observation                              | Absent distributor                                                        | Parent pending                          | Missing owner port/integration mismatch | Disabled; shed; unresolved; each qualified reason                                            |
| Weapon output collection            | Multiple identities in package order                   | Confirmed no weapons                                                      | Parent pending                          | Parent failure                          | Some/all disabled; mixed unresolved; duplicate symbols in distinct exact slots               |
...
| Unresolved-hardpoint notice         | One/multiple exact slot/symbol identities              | No unresolved entries                                                     | Parent pending                          | Coverage unavailable                    | Mixed with returned weapons; unresolved-only                                                 |
...
| Exact-slot action                   | Returned weapon exact slot                             | N/A                                                                       | Disabled while current snapshot pending | N/A: parent owns failure                | Disabled weapon action still enabled; unresolved source action; duplicate-symbol slots       |
...
- confirmed-empty and unresolved hardpoint coverage;
