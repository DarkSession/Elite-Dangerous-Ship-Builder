...
- **Contract-first dependency**: feature 005 must replace its hardpoint-only
  `HardpointPowerObservationPort` with the generalized located-mount port (T012). No feature-010 task
...
- **Package pin**: `@elite-dangerous-almanac/core@0.1.3` already contracts both `hardpoint` and
...
- [ ] T012 Replace feature 005's hardpoint-only contribution with the generalized boundary — `MountPowerObservation` retaining the `notApplicable | disabled | inactiveRetracted | powered | shed` union and package-normalized one-based `PriorityState`, `MountPowerObservationRead` carrying `buildRevision`, `conditionsRevision` and the exact slot key, and `MountPowerObservationPort.observe(context, slotKey)` accepting exact hardpoint and utility keys — in `src/app/domain/power-heat/mount-power-observation.ts`, and update the owning adapter and its injection token in `src/app/application/power-heat/mount-power-observation.adapter.ts`
- [ ] T013 Add the generalization tests proving the owner derives every utility observation from one `ShipLoadout.powerBudget()` consumer and matching band result, that an always-powered utility and a deployed-only utility yield distinct current states, that a utility absent from the consumers list reads `notApplicable` rather than powered or zero, that priority is normalized to one-based or reported unavailable, and that both revisions are stamped on every read, in `src/app/application/power-heat/mount-power-observation.adapter.spec.ts` (depends on T012)
...
- [ ] T039 [P] [US1] Implement the mount state legend covering kind, fitted and empty, engineered and stock, focused and all five power states with the same localized text used by the items, and never applying powered or shed treatment to a not-applicable mount, in `src/app/ui/hull-schematic/mount-legend.ts` and its template and styles (depends on T030)
...
- [ ] T045 [P] [US1] Add the US1 message keys — the capability heading, hull and orientation descriptions, side names and selector labels, the loading, temporarily-unavailable, offline, HTTP, network, retry and package-defect wording, every legend entry, the mount kind, size, `notClassSized`, unavailable, empty, resolved, engineered, stock, focused, priority and five power state labels, the deployed and retracted condition names, the top, bottom, both, pending, unavailable and defect location wording, the provenance action and the untranslated-text disclosure — to `src/app/i18n/locales/en.json` and `src/app/i18n/locales/de.json`
...
- [ ] T047 [P] [US1] Register the US1 surfaces and the FR-001–FR-005, FR-008, FR-010, FR-011 and FR-012 ids with their package-identity, complete-state, unique-order, defect and ledger-fallback assertions in `e2e/coverage-ledger.ts`
...
- **Foundational (Phase 2)**: depends on Phase 1 and blocks both user stories; T012 is a feature 005
  contract change that must land before any task reads a power state, and T006 must pass before the
  loader is trusted against a package upgrade
...
- Current package counts are regression fixtures for the pinned 0.1.3 release, never runtime
