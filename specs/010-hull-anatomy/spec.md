...
**Input**: Identified by design review. The imported design reads and navigates a build on the hull's
own technical schematics — top and bottom plates with every mount drawn where it physically sits. No
accepted specification covered that.
...
follows from mount geometry for a build's fire — shot convergence — belongs to
[feature 007](../007-offence-profile/spec.md), which reads the package's own catalogue of cockpit
offsets rather than anything drawn here; the illustration shown beside a hull in the catalogue
belongs to [feature 001](../001-ship-selection-and-loading/spec.md). This feature adds a second,
independent route to the same slots and a class of insight the slot list cannot carry.
...
- **FR-003**: The application MUST NOT invent, move, offset or estimate a mount position. A mount the
...
- **FR-006a**: Schematics MUST NOT delay outfitting becoming usable, as feature 001's FR-021 requires
  ...
  origin the application is served from and MUST NOT be fetched from any other (constitution
  principle I). A hull's plates MUST be cached as that hull is opened rather than precached for the
  whole catalogue, so every hull the Commander has already opened keeps its anatomy with the network
  disabled; a hull whose plates were never fetched is governed by FR-014a.
...
- **FR-017**: The slot currently focused on the build MUST be locatable on the plate, including where
  it sits on the schematic not currently in view, which MUST be brought into view rather than leaving
  the selection with no visible effect — unless a plate already in view locates it (FR-013a).
...
keys feature 002 requires, which is what makes FR-002 and FR-003 satisfiable without a private record
of any kind.
...
other seven out of the view. The cargo hatch is why that line is drawn where it is: it is a real slot
the hull has, it is marked on all 48 bottom plates and on no top plate, and it carries no slot key.
Presenting it would put a shape on the plate that a Commander would reasonably try to select, and
that FR-013 forbids resolving to the slot it plainly is. Leaving it out states the same fact more
honestly, and FR-012's coverage figure is what tells the Commander the plate does not account for
every slot.
...
rather than left to the plan: the raster cannot carry mount state, because no build exists when it is
produced (FR-002a, FR-002b), and it must be produced at a resolution that holds up at the fixed scale
FR-029 sets, on the densest supported display (FR-006d).

**The schematics carry no scale, and nothing needs them to.** There is no metres-per-unit, no overall
hull dimension and no mount coordinate in real units on any plate. That absence is what makes FR-003
...
[feature 007](../007-offence-profile/spec.md)'s shot convergence, is answered from a different
source: `SHIP_GUNSIGHTS` publishes each hardpoint's horizontal and vertical offset from the cockpit
in metres, observed in-game across all 48 hulls and 234 hardpoints, rather than measured off these
drawings. That separation is precisely what feature 007's FR-016e exists to keep. Should a future
capability need hull-relative geometry, it is raised then, by the feature that needs it.
...
- Both schematics exist for every hull in the catalogue today, so a hull without them is a temporary
  ...
  application to work when one is missing. The same holds for FR-015's mismatch case: every schematic
  slot key resolves to a real slot today, and every hardpoint and utility mount is located, so a
  mismatch is drift between a catalogue update and the artwork that follows it.
...
- The anatomy view shows one hull. Showing two hulls' anatomy side by side is out of scope, in keeping
  with the comparison feature 003's FR-004 rules out everywhere.
