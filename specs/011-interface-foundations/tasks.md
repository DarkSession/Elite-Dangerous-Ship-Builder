...
- [x] T001 Add lockfile-compatible `@jsverse/transloco` and `@angular/service-worker` dependencies and `@axe-core/playwright`, `postcss` and `postcss-scss` development dependencies in `package.json`, then install and commit the updated `pnpm-lock.yaml` — superseded 2026-08-26: Transloco was removed; the application owns interpolation in `src/app/i18n/locale-registry.ts`
...
- [x] T019 Implement the typed message facade over Transloco in `src/app/i18n/message.service.ts` and register the localization providers in `src/app/app.config.ts` (depends on T016–T018) — superseded 2026-08-26: the facade interpolates with the repository’s own `interpolate`, not Transloco
