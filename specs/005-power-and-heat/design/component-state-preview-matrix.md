...
| Component/pattern                        | Populated/default                      | Empty/unavailable                                   | Loading                                  | Error                             | Disabled/deployment                                |
| ---------------------------------------- | -------------------------------------- | --------------------------------------------------- | ---------------------------------------- | --------------------------------- | -------------------------------------------------- |
...
| Metric definition group                  | capacity/draw and deployed summaries   | omitted retracted-only definitions with explanation | skeleton/status text with stable heading | group failure text                | zero-output sentinel                               |
| Priority-band collection                 | five exact bands including zero draw   | not applicable only in no-build parent              | whole collection pending                 | projection failure                | shed and powered verdicts                          |
| Module power collection                  | multiple exact-slot numeric consumers  | no participating consumers                          | collection pending                       | missing package identity failure  | disabled, deployed-only and inactive-retracted     |
| Heat profile collection                  | three facts and five finite scenarios  | package unavailable; no-weapons equal scenarios     | profile pending                          | blocking/unexpected failure       | does-not-settle and never-overheats                |
...
| Status notice                            | retracted-summary explanation          | no notice when nothing is withheld                  | pending message                          | assertive blocking error          | non-interrupting unavailable notice                |
