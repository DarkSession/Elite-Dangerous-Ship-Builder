...
**Purpose**: Publish the cross-feature contracts features 003 and 010 compile against, then the pure
revision-stamped projection and the store every surface reads.
...
- [ ] T006 [P] Define the `HardpointPowerObservation` union (`notApplicable`, `disabled`, `inactiveRetracted`, `powered`, `shed` with package-normalized one-based priority), `HardpointPowerObservationRead` carrying both revisions and the exact slot key, and `HardpointPowerObservationPort` in `src/app/domain/power-heat/hardpoint-power-observation.ts` (contract-first export: unblocks feature 010)

### Pure projection

- [ ] T007 Define `PowerHeatProjectionState`, `PowerHeatSnapshot`, `PowerBudgetView`, `DeployedPowerSummary`, `PowerBandView`, `ModulePowerCollection`, `ModulePowerView`, `DistributorView`, `ReturnedPips`, `CapacitorView`, `HeatProfileView` and `HeatScenarioView` exactly as specified in data-model.md, with no retracted variant of `DeployedPowerSummary`, in `src/app/domain/power-heat/power-heat-projection.ts` (depends on T004)
...
- [ ] T009 Implement `projectModulePower(consumers)` producing one row per `PowerConsumerResult` with exact `label`, `symbol`, `draw`, `enabled`, normalized priority, deployed-only state and source ordinal, retaining disabled consumers, never merging identical symbols and never fabricating an absent passive or zero-draw fitting, with unit tests for duplicate symbols in different slots, disabled rows, deployed-only rows and a missing `label` or `symbol` raising the projection-failure path, in `src/app/domain/power-heat/power-heat-projection.ts` and `src/app/domain/power-heat/power-heat-projection.spec.ts` (depends on T007)
- [ ] T010 Implement `projectDistributor(loadout, conditions)` dividing each integer half-pip by two exactly once while constructing `DistributorOptions`, calling `distributorMetrics()` once, mapping `null` to `unavailable` with no capacitor figures and mapping a ready result to returned pips plus SYS, ENG and WEP `capacity`, `ratedRecharge` and `rechargeRate` in that order, with unit tests for `0/0/12`, `4/4/4` and `8/0/4` half-pips, a genuine zero recharge, an echoed pip set and a null result carrying no inferred cause, in `src/app/domain/power-heat/power-heat-projection.ts` and `src/app/domain/power-heat/power-heat-projection.spec.ts` (depends on T007)
- [ ] T011 Implement `projectHeat(loadout)` calling `heatMetrics()` once per build revision, mapping `null` to `unavailable` and a ready result to `heatEfficiency`, `hullHeatCapacity`, `hullHeatDissipation` and exactly `idle`, `thrusters`, `fsdCharging`, `firingSustained` and `firingDrained` with all five `HeatState` fields, with unit tests for the fixed order, a no-weapons build retaining five scenarios, infinite heat level, infinite gauge, null seconds to overheat and a package null profile, in `src/app/domain/power-heat/power-heat-projection.ts` and `src/app/domain/power-heat/power-heat-projection.spec.ts` (depends on T004, T007)
- [ ] T012 Implement `projectPowerHeat(context)` composing one immutable `PowerHeatSnapshot` from a single `powerBudget()` result, the distributor projection and the heat projection, stamping the captured `buildRevision`/`conditionsRevision` pair, rechecking the pair before returning and emitting `failure` with `projectionFailed` only for an unexpected exception, a missing required consumer identity or a revision-contract violation, in `src/app/domain/power-heat/power-heat-projection.ts` (depends on T008, T009, T010, T011)
- [ ] T013 Add the projection contract suite asserting `powerBudget()` is called exactly once per revision context, package `null` distributor and `null` heat remain data inside a `ready` snapshot rather than `failure`, heat is unchanged by a hardpoint or pip change, pips change only the distributor view, a newer revision discards an older outcome and no view field is derived from another, in `src/app/domain/power-heat/power-heat-projection.contract.spec.ts` (depends on T012)

### Store, presentation and repository policy

- [ ] T014 Implement `PowerHeatStore` capturing feature 001's atomic `{ loadout, buildRevision }` and feature 003's settled `{ conditions, conditionsRevision }`, memoizing `projectPowerHeat` by that revision pair in one computed signal and publishing `noBuild`, `pending`, `ready` or `failure`, with unit tests for each state, for rapid interleaved build and condition changes and for the refusal to publish a stale pair, in `src/app/application/power-heat/power-heat.store.ts` and `src/app/application/power-heat/power-heat.store.spec.ts` (depends on T012)
...
exist — features 003 and 010 can compile against this feature and user story work can begin.
...
- [ ] T025 [P] [US1] Add hardpoint observation adapter tests covering an absent consumer for the exact slot as `notApplicable`, a disabled consumer as `disabled`, retracted plus package `deployedOnly` as `inactiveRetracted`, the matching band's selected boolean as `powered` or `shed`, package-normalized one-based priority on every carrying variant, both revisions on every read and the refusal to match by index, name, symbol prefix or display string, in `src/app/application/power-heat/hardpoint-power-observation.adapter.spec.ts`
...
- [ ] T034 [US1] Implement `HardpointPowerObservationAdapter` and its injection token, resolving the exact `PowerBudget.consumers` label and matching returned band through the four ordered selection rules and stamping both revisions on every read, in `src/app/application/power-heat/hardpoint-power-observation.adapter.ts` (depends on T006, T014)
...
- [ ] T038 [US1] Add the US1 surfaces, FR-001–FR-006 and FR-011 ids, the compact status and hardpoint observation contributions, journeys and axe flags to `e2e/coverage-ledger.ts`

**Checkpoint**: The power budget, priority shedding and module contributions are independently
demonstrable, and features 003 and 010 receive owner-authored power semantics.
...
- **Foundational (Phase 2)**: depends on Phase 1 and blocks every user story; T005 and T006 also
  unblock features 003 and 010
...
- **US1 (P1)**: depends only on Phase 2. It also delivers the feature 003 and feature 010 adapters,
  because both select the same `powerBudget()` result
...
- Phase 2: T004, T005 and T006 run together; T008–T011 run together once T007 lands; T017 and T018
  run alongside T014–T016
...
Task: "Hardpoint observation adapter tests in src/app/application/power-heat/hardpoint-power-observation.adapter.spec.ts"
...
2. Complete Phase 2: Foundational — this blocks everything and unblocks features 003 and 010
