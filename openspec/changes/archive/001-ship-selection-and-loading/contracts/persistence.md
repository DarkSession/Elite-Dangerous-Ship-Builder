...
The current schemas are `LocalRecordV1` in `src/app/domain/records/local-record.ts` and
`BuildSnapshotV1` in `src/app/domain/ships/build/build-snapshot.ts`. Browser bytes are untrusted input even when this application originally wrote them.
