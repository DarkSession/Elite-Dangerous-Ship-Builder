...
them, and with no unavailable sentinel above them: the reason states the absence, and the card's
heading already says which value is missing. A refused recovery replaces its own block with the reasons the strength has not already
...
- **The states a Commander reaches by outfitting are stated in plain words.** No generator, a
  generator switched off, and a generator the plant does not feed each get a sentence of this
  application's own, chosen by the package's `field` and `reason` and by nothing else. The package's
  English sentence is never read, parsed or reproduced, and it is the one thing
  `getCalculationIssueMessage()` cannot give any locale but English. Wording a structured result is
  labelling it, which principle II sanctions.
- **Both ways the plant leaves the generator unfed share one sentence.** A generator shed for want
  of budget and a plant switched off are one thing to act on, and the plant's own state is feature
  005's to report. They stay two package diagnoses on the projection.
...
| no generator / switched off        | The capability's own sentence for that exact field and reason, and nothing in its place |
| any way the plant does not feed it | One shared statement of insufficient power, and nothing in its place                    |
