...
A reason states the absence, so no unavailable sentinel stands beside it. `Shield strength ·
Unavailable · no shield generator is fitted` said the same thing twice under a heading that already
says which value is missing. There is always a reason to say it: the package types an incomplete
result as carrying one issue at least.

The same rule takes the recovery's sentinel with its issues. A recovery with no reason of its own
would state a second absence for the same fault, so it is left out of the region entirely; a
recovery refused by anything else states what refused it.
...
English sentence. The pair is what a surface is given to act on, and these are read from the pair:
...
| `shieldGenerator` / `shed`     | insufficient power for the shield generator |
| `powerCapacity` / `disabled`   | insufficient power for the shield generator |
...
`getCalculationIssueMessage()` answers for English and returns `null` for every other locale,
whatever the reason. It is not a private translation of the package's sentence — that sentence is
never read, parsed or reproduced here.

The last two share a statement on purpose. A generator shed for want of budget and a plant switched
off are two package diagnoses, and the projection keeps them two; what a Commander is told is the
thing they have in common, because the shields are down for one reason either way and there is one
thing to do about it. The plant's own state is feature 005's to report.

The plant cannot be absent: `ShipLoadout` refuses to empty the `PowerPlant` slot, so a
`powerCapacity` / `missing` is not a state this application can reach and has no row of its own. It
would keep the package's own words like any other pair the table does not name.

The first three rows stay apart from each other, which is what FR-003 asks: nothing fitted, switched
off and unfed are three different things to do about it.
...
the four above and an `unresolved` record are five package diagnoses and read as four statements,
the two ways the plant leaves the generator unfed sharing one.
...
- Prove a generator that is not fitted, one switched off and one the plant does not feed read as
  three different sentences, and that the two ways the plant leaves it unfed read alike.
- Prove a diagnosis with no entry in the table keeps the package's own words.
