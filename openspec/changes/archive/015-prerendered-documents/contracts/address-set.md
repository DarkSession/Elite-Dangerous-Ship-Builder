...
`discoverRoutes: false` so it is the whole set. `outputMode` MUST NOT be set.
`angular.json` cannot carry a comment saying so, and `outputMode: 'static'` reads
like the obvious choice: it silently discards the routes file
(`@angular/build/src/builders/application/options.js`, which warns that
`prerender` "is not considered" when `outputMode` is specified) and moves route
selection into an `app.routes.server.ts` using `RenderMode.Prerender`. That
enumerates the 48 hulls a second time, in application source, and leaves the
policy checker two address definitions to reconcile instead of the one derived
list FR-006 and FR-021 rest on. With no `outputMode`, `ignoreServer` is true and
no server bundle is written to the output, so the server entry is a build-time
renderer and never a deployed artefact — which is what makes the configuration
honest against constitution I.
...
## 5. What the address set costs, and why no gate measures it

Measured from the shipped build. A hull document is large because `/ships/:hull`
is a child of `/ships`, so it legitimately contains the catalogue behind the
inspector — that is what is on screen at wide widths.
...
14.46 MB of raw HTML enters the Pages artefact across all 54 files, served at
roughly 22 KB each compressed. The shell is 29 KB rather than the 5.5 KB the
spike measured: that is the hydration payload plus the per-address head, and it
is the price of the takeover not blanking the page.

The `initial` bundle went from 516.79 kB raw / 122.68 kB compressed to
547.99 kB / 132.27 kB — all of it Angular's hydration and event-replay runtime.
The `initial` budget warns at 500 kB and errors at 1 MB. The build **already
warned before this feature**, and still only warns; that warning is expected and
was accepted knowingly.

No size gate measures HTML, and none is added. About 1.1 MB of compressed HTML
across the whole set does not earn a mechanism.
