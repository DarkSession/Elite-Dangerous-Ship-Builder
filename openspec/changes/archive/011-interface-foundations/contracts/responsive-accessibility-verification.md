...
Locale switching is gated structurally, not on a stopwatch: English makes zero
locale requests, a cold secondary locale makes at most one same-origin request, a
warm switch makes none, formatter construction is cached by locale, options and
kind, and each switch produces one committed snapshot. A wall-clock threshold is
deliberately refused. The earlier 100 ms/4x-CPU target was an invented,
environment-sensitive requirement that no specification asks for, and a
cross-browser stopwatch gate is flaky until a measurable latency target exists.
Request counts, cache construction and atomic revision behaviour are
deterministic and protect the same experience.
