...
| `658 T OF HEADROOM` under the ranges                                   | Keep     | Withdrawn 2026-08-26. The row was removed as a comparison nothing publishes; it is one the package answers twice over. `658` is the canvas's own `1,800` optimal mass less its own `1,142` loaded mass, both already read here, and the subtraction is the whole reading: how much more this build can take on before the drive stops jumping the way the row above says it does. Restating two figures is exactly what a headroom is, and a Commander fitting a module is asking for it (Commander request 2026-08-26). Drawn only while there is headroom; at or over the optimal mass the note says how far over instead, because `-120 t of headroom` is not a sentence.                                                                                                                                                          |
...
| The `OPTIMAL` / `MAX` labels as absolutely positioned text             | Keep     | Both stay under the positions they mark — the optimal centred on its own tick, the maximum against the end of the track — and both stay on the one line under the bar, which is where the canvas draws them. An earlier pass put them in the flow of a wrapping row so that a doubled text size in German could drop the maximum to a line of its own rather than let the two meet. Measured, that wrap was not the exception but the rule: at the width this card is given the pair is over by five pixels on an ordinary build, so the maximum sat two lines under the track it is the end of (Commander request 2026-08-26). The two share one grid cell instead — one line at every width — and the optimal's label wraps within its own box before it can reach the maximum. See `mobility-and-jump-profile.md`, "The mass bar". |
...
| `SPEED ENVELOPE AT THIS MASS`, over the five envelope rows             | Unheaded | Drawn until 2026-08-26, when the Commander asked for it back off. It is a line of prose over five rows that each name themselves, on the longer of the two cards, and the condition it states — `at this mass` — is on the headline directly above it. The words stay as the list's accessible name, so nothing is lost to a reader moving by list and a line of the card's height is returned to the rows that carry figures.                                                                                                                                                                                                                                                                                                                                                                                                        |
| The headline trio's three cells                                        | Keep     | `repeat(3, 1fr)` at every width, as the canvas sets them. Sized by the card's own width instead, the trio came out two across with `Mass lock` alone under them — an arrangement neither artboard draws, on a card wide enough for three (Commander request 2026-08-26). Feature 005's priority footer is the same three-cell shape at the same measurements and takes the same modifier.                                                                                                                                                                                                                                                                                                                                                                                                                                             |

### Added to it

| Added element                                                  | Reason                                                                                                                                                                                                                                                                   |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Switched off`, on a mount the outfitting panel has turned off | Neither canvas has an off state to draw, and the readings a switched-off mount produces are not the ones beside it. Feature 005 records the same addition for the same reason (`openspec/changes/archive/005-power-and-heat/design/reference-review.md`, "Added to it"). |
...
specification is what gets corrected; FR-004 and FR-006 in [spec.md](../../../../specs/ship-builder/mobility-and-jump/spec.md) carry those
corrections.
