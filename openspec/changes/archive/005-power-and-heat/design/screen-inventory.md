...
| Status rail shed sentence (feature 003's rail)   | In the opening `BUILD STATUS` block, under the validation issues                  | The same sentence in the same block of canvas 1d's Status mode                                                                                                                                        | FR-013                 |
| Status rail power block (feature 003's rail)     | Its own block, over the metric cells                                              | The same block in canvas 1d's Status mode                                                                                                                                                             | FR-013                 |
| Key-figure power badge                           | Not drawn: the rail beside the workspace carries the whole line                   | Against the trailing edge of canvas 1d's strip of key readings, where a priority group is dark — the share of plant output where the plant has output, over one line naming each dark group           | FR-014                 |
...
| FR-014      | The compact strip's badge is drawn only where a priority group is unpowered, names each such group from the same `powerBudget()` result the rail reads, states the lit share of plant output above them where the plant has output, and is inert.                                                           |
