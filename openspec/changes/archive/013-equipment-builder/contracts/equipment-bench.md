...
| material requirement                                                             | `getPersonalModificationCost` per fitted **unlocked** modification, plus `getSuitUpgradeCost` and `getPersonalWeaponUpgradeCost` for the climb to each selected grade, through `sumPersonalEngineeringIngredients` (FR-013, FR-014). Both upgrade calls answer `null` for an item with no ladder and `[]` for a climb already made; the bench holds no item the catalogue did not name, so neither answer is a missing figure and both add nothing |
