...
- [x] 1.2 Add to `e2e/prerendered-first-frame.spec.ts`, in the journey "states a first navigation
...
- [x] 1.3 Add a unit case beside `src/app/app-navigation-waiting.spec.ts` driving the same
...
- [x] 2.1 Add the adapter that copies the served content, under `src/app/platform/browser/`,
...
- [x] 2.6 Verify the held content carries the language it was served in, while the application
...
- [x] 3.1 Verify a navigation that fails after a screen has been presented leaves the Commander on
...
- [x] 3.2 Verify a first navigation that is cancelled or redirected, and whose replacement then
...
- [x] 3.3 Verify an address the build generates no document for is unaffected: nothing is held, and
...
- [x] 3.4 Verify held content is kept as the address served it: the same nodes, with no figure
...
- [x] 3.5 Verify a first navigation cancelled with nothing taking over leaves the Commander on what
...
- [x] 4.1 Verify the preview manifest needs nothing added, rather than adding a state to it.
      ...
      constitution IX).
- [x] 4.2 Scan the held composition where it stands, in the production lane, across the layout
...
- [x] 5.4 Register the change in `e2e/coverage-ledger.ts`: add `023-served-document-held` to
