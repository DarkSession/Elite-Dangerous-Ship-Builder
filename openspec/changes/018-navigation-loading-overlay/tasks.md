...
- [x] 1.1 Add a `@media (prefers-reduced-motion: reduce)` block to the `<style>` inside
...
- [x] 1.2 Add a rule to `scripts/check-interface-foundations.mjs` asserting the shared
...
- [x] 1.3 Add the softened scrim to the token layer: `rgb(6 6 7 / 0.55)` as a primitive
...
- [x] 2.2 State the threshold as one named constant of 10 milliseconds in that file, citing
      ...
      above (018/FR-004).
- [x] 2.3 Raise a second signal when a navigation fails, and lower it on the next navigation
      ...
      FR-007).
- [x] 2.4 Suppress the waiting signal, and only the waiting signal, for the navigation that
...
- [x] 3.1 Add `ednb-waiting-overlay` in `src/app/ui/components/waiting-overlay/`: a
...
- [x] 4.1 Add the waiting sentence and the failed-navigation notice — its message and its
...
- [x] 4.2 Widen the application frame's status slot from one notice to a list, drawn in
...
- [x] 4.3 Mount the overlay in `src/app/app.html` beside the frame, under the same
...
- [x] 5.1 Assert in the shell's tests that a navigation resolved inside the threshold draws
      nothing, and that a navigation held open draws the overlay over whatever screen was on
      (018/FR-004).
- [x] 5.2 Assert that the overlay carries no control and that the native cancel does not
      close it, so the only thing that removes it is the navigation ending (018/FR-003,
      FR-005).
- [x] 5.3 Confirm coverage stays at or above the 80% floor `angular.json` enforces, by
