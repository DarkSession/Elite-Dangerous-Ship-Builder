...
The overlay is mounted under the browser-only condition the help modal is mounted under, so a
document generated by the build never carries it. The notice reaches the frame unconditionally,
as every status notice does; what keeps it out of a generated document is that the render runs
one navigation and that navigation does not error. In a browser, the store
