...
The movement is small: about seven pixels for the ladder, and four more where the empty bench
and the chosen item disagree about the seam under the heading. It is enough to make a
Commander lose their place in the list. On a second press it is enough to put a different row
under the pointer than the one that was there when the press began.

## What Changes

- The space the grade ladder takes is held whether or not the selected item publishes a grade.
  Choosing an item fills that space rather than making it.
- The held space is empty. It draws nothing, is hidden from the accessibility tree and is not
  a control, so nobody is handed a grade control for an item that has no grade.
- The space is held where the ladder stands beside the item's name. Where the column is narrow
...
- The grade ladder's own composed height enters the token layer. Composed from the ladder's
  parts rather than declared as a figure, a drift between the held space and the ladder
...
- **FR-002** Space held for a grade choice the selected item does not offer states nothing.
...
- **An empty mount offers no grade to press.** That is `equipment-builder/loadout-assembly`,
  ...
  carry — that space held where the ladder would stand is not announced and is not a control.
...
- `equipment-builder/loadout-assembly`: gains two requirements. The first states that the list
  ...
  the first suit is chosen at the empty bench, and between one selected item and the next. The
  second states what space held for an absent grade choice may say, which is nothing.
...
- `src/styles/tokens/_semantic.scss` gains the grade ladder's composed height as a token,
  built from the ladder's own parts so that changing the ladder changes the held space with
  it.
- `src/app/features/equipment/item-view/item-view.spec.ts` states that the empty track is
  drawn, that it carries no ladder, and that it is hidden from the accessibility tree and the
  focus order. `src/app/features/equipment/suit-gate/suit-gate.spec.ts` gains no case: the
...
- `e2e/equipment-builder.spec.ts` measures the journeys the requirements are about: fitting a
  weapon into an empty mount, choosing the first suit at the empty bench, opening an empty
  mount after a fitted item, and the same at the narrow arrangement.
