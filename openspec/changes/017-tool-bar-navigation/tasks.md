...
- [ ] 1.1 Add the mark's destination to both message catalogues — the name a reader
      hears for the way to the entry point — and verify with `pnpm run policy`, which
      fails on a key missing from either catalogue.
- [ ] 1.2 Make `AppNavigation.home()` answer with the entry point from every screen,
      ...
      address.
- [ ] 1.3 Draw the mark as a link on every screen in `app-frame.html`, keeping the
...
- [ ] 1.4 Return from `App.navigateFromShell` without navigating when the entry's
...
- [ ] 2.1 Add the re-entry to `ToolRecord` — the tool's own address by default, or a
      ...
      action.
- [ ] 2.2 Draw every tab as a link in `app-frame.html`, with `aria-current="true"` on
      ...
      staying green.
- [ ] 2.3 Dispatch a tab that declares an action through the shell instead of
...
- [ ] 2.4 Update `e2e/design-reference.spec.ts`, which reads the mark as decoration on
      the shipyard and the current tool as a word. Verify by running the design
      reference project.

## 3. One autosave, two tools

- [ ] 3.1 Introduce the port `AutosaveService` and `TabOwnershipCoordinator` read —
      ...
      green.
- [ ] 3.2 Give `LocalRecordRepository.findUnnamedMatching` the tool it is matching, so
...
- [ ] 3.3 Move `TabDescriptorRepository` to version 2 — one working record id per tool
...
- [ ] 3.4 Let `TabOwnershipCoordinator` claim, announce and fork one identity per tool,
...
- [ ] 4.1 Add to `LoadoutStore` what autosave reads: a fingerprint over
...
- [ ] 4.2 Add the equipment adapter for the autosave port in
      ...
      a full store.
- [ ] 4.3 Start ownership, restoration, ingress, publication and autosave on the bench
      ...
      out.
- [ ] 4.4 Pause autosave and keep the loadout usable when another page deletes the
      record this bench writes to, resuming on an explicit action. Verify with a unit
      test over the deletion broadcast.
- [ ] 4.5 Give `ednb-persistence-status` its state and paused flag as inputs, report
...
- [ ] 4.6 Confirm `pnpm run policy` still passes `equipment-ownership.mjs` with the new
...
- [ ] 5.1 Add the application-layer action that empties the bench: clear the loadout,
...
- [ ] 5.2 Clear the loadout from the address when the bench empties, replacing the
      fragment rather than adding a history entry. Verify with a unit test over the
      publisher with no loadout on the bench.
- [ ] 5.3 Wire the equipment tool's re-entry action to it in `App.selectAction`, so the
...
- [ ] 6.1 Add the end-to-end journey for the mark: reach the entry point from a hull,
...
- [ ] 6.2 Add the end-to-end journey for the tabs: the ship tool's tab from a hull and
      ...
      standing.
- [ ] 6.3 Add the end-to-end journey for the bench's autosave: assemble a loadout,
      reload, and find it restored; then start an empty bench and open the loadout
      again from the saved records layer.
- [ ] 6.4 Add `017-tool-bar-navigation` to `COVERED_FEATURES` in
...
- [ ] 6.5 Scan the bar and the bench with axe at all five layout profiles in both
      engines, with the open tool drawn as a control, and assert zero violations of
      the in-scope criteria.
- [ ] 6.6 Record a screen-reader pass over the bar in `e2e/manual/results/`, covering
